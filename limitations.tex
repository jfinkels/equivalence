...
\begin{construction}\label{con:rands}
  ...
  For each positive integer $n$, let $R_n$ be the set of all strings of length $n$ and let $S_n$ be the set of all strings $t$ satisfying the inequality $p_n(n) + 1 \leq |t| \leq p_{n + 1}(n + 1)$ (except $S_1$, which must also include all strings of length $1$).
...
\begin{lemma}
  $R$ and $S$ are equivalence relations on $\Sigma^* \setminus \{\lambda\}$.
\end{lemma}
\begin{proof}
  $R$ and $S$ are equivalence relations if $\{R_n\}_{n \in \mathbb{Z}^+}$ and $\{S_n\}_{n \in \mathbb{Z}^+}$ are valid partitions of $\Sigma^* \setminus \{\lambda\}$, so it suffices to show that each set in the collection is nonempty, the union of each collection includes all nonempty strings in $\Sigma^*$, and each collection is pairwise disjoint.

  The set $R_n$ includes at least one string of length $n$, so each $R_n$ is nonempty.
  Any string of length $n$ is in $R_n$, so $\Sigma^* \setminus \{\lambda\} \subseteq \cup_n R_n$.
  ...
  Next, for any string $x$, there exists an $n$ such that $p_n(n) + 1 \leq |x| \leq p_{n + 1}(n + 1)$, so every nonempty string in $\Sigma^*$ is in some $S_n$.
...
\begin{theorem}\label{thm:different}
  There are equivalence relations $R$ and $S$ on $\Sigma^*$ such that $R \mor S$ but $R \nkr S$.
...
This idea was presented, independently, in \autocite[Lemma~2.3]{gz14}.
...
\begin{proof}[Proof of \autoref{thm:different}]
  ...
  On input $\pair{x}{y}$, if $|x| = |y|$, output $\pair{a}{a}$, otherwise output $\pair{a}{b}$, where $a$ is a string in $S_1$ and $b$ is a string in $S_2$.
  ...
  Since $f_0, f_1, \dotsc$ is an enumeration of all polynomial time computable functions, the reduction from $R$ to $S$ is $f_n$, with running time $p_n(n)$, for some positive integer $n$.
  ...
  The theorem is true for equivalence relations on $\Sigma^* \setminus \{\lambda\}$, now we need to consider the empty string.
  Let $R' = R \cup \{(\lambda, \lambda)\}$ and $S' = S \cup \{(\lambda, \lambda)\}$.
  The only difference in the above argument is that
  \begin{equation*}
    \#R'(n) = \#R(n) + 1 = n + 2
  \end{equation*}
  and
  \begin{equation*}
    \#S'(p_n(n)) = \#S(p_n(n)) + 1 = n + 1.
  \end{equation*}
  The inequality $\#R'(n) > \#S'(p_n(n))$ still holds, so we have produced the same contradiction.
\end{proof}
