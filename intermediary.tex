...
\begin{construction}\label{con:diag}
  ...
  For this we need an enumeration of each polynomially clocked deterministic Turing machine, $\{M_i\}_i$, where machine $M_i$ halts within time $i n^i$ on inputs of length $n$.
...
\begin{lemma}\label{lem:loglog}
  The function $p$ in \autoref{con:diag} is computable in time polynomial in $n$.
\end{lemma}
\begin{proof}
  Computing $p(n)$ requires computing $p(1)$, $p(2)$, $\dotsc$, $p(n - 1)$; if there is a polynomial that bounds the running time of each of these $n - 1$ computations, the total time required to compute $f(n)$ remains polynomial in $n$, by induction.
  ...
  \begin{equation*}
    i (\log \log n)^i \leq \log \log n (\log \log n)^{\log \log n} \leq 2^{(\log \log n)^2} \leq 2^{O(\log n)} = \poly(n),
  \end{equation*}
...
\begin{proof}
  ...
  The machine $M_i$ never disagrees with $R$, so $p(n) = n^i$ for all sufficiently large $n$.
...
\begin{proof}
  ...
  Each recursive invocation of the algorithm other than the base case requires computing $f$ on an input of length $n$ (twice), which can be done in polynomial time, and deciding if the lengths of $f(x)$ and $f(y)$ are in the range of $p$ can be done in polynomial time by \autoref{lem:loglog}.
  ...
  Combining these three relations yields the inequality $|x'|^k = p(|x'|) = |f(x)| \leq |x|^j$, so $|x'| \leq |x|^{j / k} < |x|$, since $k > j$ and lengths must be natural numbers.
\end{proof}
