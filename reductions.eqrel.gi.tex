...
\begin{proof}
  ...
  \ref{thm:req_kr_dirgi}, creates a ``spine'' representing each bit of word $w$.
...
\begin{proof}Follows directly from Theorem \ref{thm:reqi_kr_dirgi} and Corollary \ref{cor:gi_ke_dirgi}.\end{proof}

\begin{theorem}\label{thm:ra_kr_dirgi}$\forall a\in\sigmastar R_a \kr
  DirGI$\end{theorem}
\begin{proof}
	...
	\begin{algorithm}[H]
		...
		$F_w\gets\{(v_1, v_2), (v_2, v_3), \ldots, (v_{n-1}, v_n)\}$\tcp*[f]{directed
			edges}\;
	...
	the disjoint union of two distinct graphs, one representing the bits of $w$ and
	the other representing the bits of $w\oplus a$.
  ...
  $G_x$ is isomorphic to $G_y$, and hence $(M(x), M(y))\in DirGI$.
	...
	$y_i\olpus a_i=1\oplus 0=1$, $M(y)$ produces graph $G_y$ containing vertex
