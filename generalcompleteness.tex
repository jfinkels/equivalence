...
This last question leads us to briefly note that equivalence of true quantified boolean formulas is a \PSPACE-complete problem.
Perhaps it is also a \PSPACEEq-complete problem.
\begin{proposition}
  Define $\QBFEq$ by
  \begin{displaymath}
    \QBFEq = \{\pair{\phi}{\psi} | \phi \plain{and}\, \psi \plain{are true, and}\, \phi \iff \psi\},
  \end{displaymath}
...
\begin{proof}
  ...
  There $\phi$ is satisfiable if and only if $\exists z\colon\overline{Q}(\tau\land z)$ is satisfiable.
  Therefore $\QBF\mor\QBFEq$, so $\QBFEq$ is \PSPACE-complete.
\end{proof}
