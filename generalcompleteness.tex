...
This complements the work on ``potential reducibility'' found in section 6 in \cite{bcffm}.
...
It is obvious that for any equivalence relations $R$ and $S$, $R\kr S\implies R\pot S$, and hence $R\npot S \implies R\nkr S$.
We provide a definition of ``kernel sparseness'', and show its application to determining potential reducibility and hence kernel reducibility.
...
\end{proposition}
\begin{proof}
  Assume with the intention of producing a contradiction that $R\pot S$.
