...
\begin{theorem}\label{thm:npeqimpliesnpc}
  If an equivalence relation is $\NPEq$-complete, then it is $\NP$-complete.
\end{theorem}
\begin{proof}
  Let $R$ be an equivalence relation and suppose $R$ is $\NPEq$-complete.
  Define the set $A$ by
  \begin{equation*}
    A = \left\{ \pair{G}{H} \, \middle| \, \pair{G}{H} \in \textsc{GI} \text{ or } G \text{ and } H \text{ have a Hamiltonian cycle}  \right\}.
  \end{equation*}
  It suffices to prove that $A$ is an $\NP$-complete equivalence relation.
  If this were true, then $A \kr R$ by hypothesis, which implies $A \mor R$, and therefore $R$ is also $\NP$-complete.

  It is straightforward to prove that $A$ is an equivalence relation.
  It is in $\NP$ because the graph isomorphism problem and the Hamiltonian cycle problems are both in $\NP$.
  It is $\NP$-hard by the reduction $G \mapsto \pair{G}{C_{n + 1}}$, defined for each graph $G$ on $n$ vertices, where $C_{n + 1}$ is the cycle graph on $n + 1$ vertices.
  If $G$ has a Hamiltonian cycle, then both $G$ and $C_{n + 1}$ have one.
  If $G$ has no Hamiltonian cycle, then $G$ is not isomorphic to $C_{n + 1}$ (since they have a different number of vertices) and it is not true that both $G$ and $C_{n + 1}$ have a Hamiltonian cycle.
  Therefore $A$ is an $\NP$-complete equivalence relation.
\end{proof}

This theorem provides a clearer proof of \autocite[Proposition~8.1]{bcffm}.
...
\begin{proof}
  ...
  Let $A$ denote the $\NP$-complete equivalence relation defined in the proof of \autoref{thm:npeqimpliesnpc}.
...
\begin{theorem}\label{thm:inj}
  ...
  \begin{equation*}
    A = \left\{ \pair{G}{H} \, \middle| \, \pair{G}{H} \in \textsc{GI} \text{ or } G \text{ and } H \text{ have a Hamiltonian cycle and } |G| = |H|  \right\}
  \end{equation*}
