...
Suppose $\mathcal{K}$ is some set of structures in which
\begin{enumerate}
...
\begin{example}
  ...
  In this case, $\IsoK=\GI$, the well-studied graph isomorphism problem.
\end{example}
...
(Note that the requirement that $|X|=|Y|$ only changes the ``$\Pi(X)$ and $\Pi(Y)$ are both true'' part of the definition of $A(\Pi)$, since $X\cong Y$ implies $|X|=|Y|$ by our initial assumptions.)
