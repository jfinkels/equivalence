...
We claim without proof that $\Pi$ is an \NP-complete property only if it is not a null property.
...
\end{proposition}
We use this to show that there are \NP-complete equivalence relations.
...
\begin{proposition}\label{prop:APieq}
  If $\Pi$ is a non-null, uniform, \NP-complete property, then $A^=(\Pi)$ is an \NP-complete equivalence relation.
\end{proposition}
\begin{proof}
...
\begin{theorem}
  Let $\Pi$ be a non-null, uniform, \NP-complete property.
...
\begin{proof}
  ...
  Since $B$ is \NPEq-complete, $R\kr A$.
  Thus there exists a polynomial time computable function $g$ such that $\pair{x}{y}\in\R$ if and only if $\pair{g(x)}{g(y)}\in A$.

  Let $w\in\sigmastar$.
  Then $g(w)=G$ for some graph $G$.
  By the above arguments, $[w]_R$ is infinite and $[g(w)]_A$ is finite.
  By \autoref{lem:image}, $g([w]_R)\subseteq [g(w)]_A$.
  Consider $g|_{[w]_R}$, that is, $g$ restricted to the domain $[w]_R$.
  Then $g|_{[w]_R}$ is a mapping from the infinite set $[w]_R$ to the finite set $[g(w)]_A$.
