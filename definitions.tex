...
\end{figure}
\begin{figure}
  \caption{\label{fig:inclusionscollapsed}The inclusions from \autoref{fig:inclusions} with equalities collapsed.}
  \begin{displaymath}
    \xymatrix{%
      \NPEqSeven \ar[r] & \NPEqFive \\
      \NPEqEight \ar[u] \ar[r] & \NPEqSix \ar[u]
    }
  \end{displaymath}
\end{figure}
...
\begin{sketch}\mbox{}
  ...
  \item
    For the forward direction, define $L$ by
    \begin{displaymath}
      L=\lb\pair{\pair{x}{y}}{\pair{w_x}{w_y}}\st\pair{\pair{x}{w_x}}{\pair{y}{w_y}}\in R'\rb,
    \end{displaymath}
    so the witness that $\pair{x}{y}\in L$ is the pair $\pair{w_x}{w_y}$.
...
\autoref{fig:inclusionscollapsed} shows the inclusions among these classes with the equalities from \autoref{thm:definitions} collapsed.
