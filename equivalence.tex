...
\usepackage[pdftitle={Reductions among equivalence problems in the polynomial hierarchy}, pdfauthor={Jeffrey Finkelstein}]{hyperref}
...
\title{Reductions among equivalence problems in the polynomial hierarchy}
\date{\today} 
...
\abstract{%
  ...
  We show that there is a complete problem under kernel reductions in \coNPEq.
...
In general, the class \defn{$\mathcal{C}Eq$} is the class of languages induced by equivalence relations which are in the complexity class $\mathcal{C}$.
...
\begin{langdef}{EQUIV}
  \begin{instance}
    a polynomially clocked $\SPP$ Turing machine and a positive integer $n$
  \end{instance}
  \begin{question}
    Is the set $\{\pair{x}{y}\subseteq{(\Sigma^{\leq n})}^2|M$ accepts $\pair{x}{y}\}$ an equivalence relation?
  \end{question}
\end{langdef}

\begin{lemma}
  If $\lang{EQUIV}\in\SigPi$ and $A$ is the $\SigPi$ machine which decides \lang{EQUIV}, then the language
  % TODO replace \,\, with a single space command
  \begin{align*}
    R = {} & \{\pair{u}{v}|u=v\} \\
    & \cup \{\pair{\triple{M}{x}{1^{t_x}}}{\triple{M}{y}{1^{t_y}}} | \\
    & \qquad M \plain{is a polynomially clocked Turing machine of type}\, \SigPi \\
  ...
  \end{align*}
  is complete for $\SPPEq$ under $\kr$ reductions.
\end{lemma}
...
\begin{proof}
  Testing for equality is trivially in $\SigPi$.
  Since both $A$ and $M$ are $\SigPi$ machines, simulating $A$ two times and $M$ one time is in $\SigPi$.
  Hence, $R\in\SigPi$.
  ...
  Since $M$ accepts $\pair{x}{y}$, and since $M$ accepts an equivalence relation on inputs of size at most $\max(|x|, |y|)$ (which includes the strings $x$ and $y$ themselves), $M$ must also accept $\pair{y}{x}$.
  ...
  Since $M$ accepts $\pair{x}{y}$ and $\pair{y}{z}$, and since $M$ accepts an equivalence relation on inputs of size at most $\max\{|x|, |y|, |z|\}$, $M$ must also accept $\pair{x}{z}$.
  Since $R$ is reflexive, symmetric, and transitive, it is an equivalence relation.

  Finally we must show that every equivalence relation $S$ in $\SPPEq$ reduces to $R$ under a polynomial time kernel reduction.
  Let $p$ be the polynomial which bounds the running time of $A$.
  Suppose $S\in\SPPEq$, let $M$ be the polynomially clocked oracle Turing machine which decides $S$.
  We claim that $S$ reduces to $R$ by the mapping $x\mapsto \triple{M}{x}{1^{p(|\pair{M}{x}|)}}$.
  Call this mapping $f$.
  ...
  Suppose $\pair{x}{y}\in S$, so that $f(x) = \triple{M}{x}{1^{p(|\pair{M}{x}|)}}$ and $f(y) = \triple{M}{y}{1^{p(|\pair{M}{y}|)}}$.
  First, since $S\in\SPPEq$, $M$ is a $\SigPi$ machine.
  Second, since $S$ is an equivalence relation, $A$ will accept $\pair{M}{|x|}$ within $p(|\pair{M}{x}|)$ steps and $\pair{M}{y}$ within $p(|\pair{M}{y}|)$ steps.
  Third, since $M$ decides $S$, $M$ accepts $\pair{x}{y}$.
  Thus $\pair{f(x)}{f(y)}\in R$.

  Suppose $\pair{x}{y}\notin S$, and $\pair{f(x)}{f(y)}$ are defined as above.
  ...
  We have shown that $R$ is both \SPPEq-hard and in \SPPEq, so $R$ is \SPPEq-complete.
\end{proof}

\begin{theorem}
  $\SPPEq$ has a complete problem under polynomial time kernel reductions.
\end{theorem}
\begin{proof}
  We provide a $\SigPi$ algorithm which decides \lang{EQUIV}, thereby satisfying the hypothesis of the previous lemma.
  The algorithm will decide whether a $\SigPi$ machine $M$ decides an equivalence relation on $\Sigma^{\leq n}$.
  ...
  \end{enumerate}
  Since $L(M)\in\SigPi$ and $\SigPi$ is closed under complement, $\overline{L(M)}\in\SigPi$.
  Let $M'$ be the machine which decides $\overline{L(M)}$.
  Now our conditions can be expressed as
  \begin{enumerate}
  \item $M$ accepts $\pair{x}{x}$
  \item $M'$ accepts $\pair{x}{y}$ or $M$ accepts $\pair{y}{x}$
  \item $M'$ accepts $\pair{x}{y}$ or $M'$ accepts $\pair{y}{z}$ or $M$ accepts $\pair{x}{z}$
  \end{enumerate}
  ...
  \item Simulate $M'$ on $\pair{x}{y}$ and $M$ on $\pair{y}{x}$.
    If they both reject, reject.
  \item Simulate $M'$ on $\pair{x}{y}$, $M'$ on $\pair{y}{z}$, and $M$ on $\pair{x}{z}$.
    If all three reject, reject.
  \item If execution reaches this point, accept.
  \end{enumerate}

  This is a $\SigPi$ algorithm: since this algorithm consists of a constant number of simulations of $M$ and $M'$, which are $\SigPi$ machines, specifically $\Pi_2P$ machines, prepending a universal quantifier (for strings of size at most $n$) does not affect the complexity of the algorithm.
  It remains to show that the algorithm is correct.

  If the algorithm rejects, the machine $M$ has failed on some requirement for equivalence relations on strings of length at most $n$---reflexivity if it rejects in step 2, symmetry if it rejects on step 3, or transitivity if it rejects on step 4.
  Therefore $\pair{M}{n}$ must not be in \lang{EQUIV}, and the algorithm has correctly rejected.
  If the algorithm accepts, the machine $M$ has passed all the requirements for accepting an equivalence relation on strings of length at most $n$, so $\pair{M}{n}\in\lang{EQUIV}$ and the algorithm was correct as well.
  This concludes the proof that $A$ is a correct $\SigPi$ algorithm for \lang{EQUIV}.
\end{proof}
...
Of course, under this hypothesis the polynomial hierarchy collapses, so the \SPPEq-complete problem exhibited in \autoref{thm:completeproblem} is also an \NPEq-complete problem.
\begin{openproblem}
