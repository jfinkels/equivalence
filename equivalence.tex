...
%% This is strongly recommended by biblatex.
\usepackage{csquotes}
\usepackage[backend=biber, style=tocplain]{biblatex}
\usepackage{amsmath}
\usepackage{amssymb}
...
% running title, in case the title is very long
\runningtitle{Polynomial time kernel reductions}
% plain text title, in case the title has LaTeX commands
\tocpdftitle{Polynomial time kernel reductions}

% author names
\runningauthor{Jeffrey Finkelstein and Ben Hescott}
\copyrightauthor{Jeffrey Finkelstein and Ben Hescott}
% author names in plain text
\tocpdfauthor{Jeffrey Finkelstein and Ben Hescott}
...
\begin{frontmatter}
  \title{Polynomial time kernel reductions}
  %\date{\today}
  \author[jeffrey]{Jeffrey Finkelstein}
  \author[ben]{Ben Hescott}
  \input{abstract}
  \tocacm{F.1.3}
  \tocams{03D15, 68Q15, 68Q17}
  \tockeywords{equivalence relations, polynomial time reductions, kernel reductions, kernel completeness}
\end{frontmatter}
\symbolfootnote{%
