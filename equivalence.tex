...
\begin{proof}
  ...
  We claim this is a $\SigPi$ algorithm.
  Since $\SigPi$ is closed under complement, deciding if $M$ rejects on an input is also in $\SigPi$.
  ...
  This concludes the proof that $A$ is a correct $\SigPi$ algorithm for \lang{EQUIV} running in time $n^{f(|M|)}$.
\end{proof}
