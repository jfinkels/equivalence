...
\usepackage[pdftitle={Reductions among equivalence problems in NP and coNP}, pdfauthor={Jeffrey Finkelstein}]{hyperref}
...
\title{Reductions among equivalence problems in \texorpdfstring{\NP}{NP} and \texorpdfstring{\coNP}{coNP}}
\date{\today} 
...
\begin{theorem}\label{thm:conpeqc}
...
\begin{proof}
  ...
  Since $M$ is in $\coNP$, adding three more universal quantifiers does not change the complexity of running this simulation.
  ...
  Since $M$ accepts $\pair{x}{y}$ in at most $\max(t_x, t_y)$ steps and $\pair{y}{z}$ in at most $\max(t_y, t_z)$ steps, and since $L(M)$ is an equivalence relation on strings of size at most $\max\{|x|, |y|, |z|\}$, $M$ must also accept $\pair{x}{z}$.
  \begin{remark}
  ...
  We claim that $S$ reduces to $R$ by the mapping $x\mapsto \triple{M}{x}{1^{p(|\pair{x}{x}|)}}$.
  Call this mapping $f$.
  ...
  Suppose $\pair{x}{y}\in S$, so that $f(x) = \triple{M}{x}{1^{p(|\pair{x}{x}|)}}$ and $f(y) = \triple{M}{y}{1^{p(|\pair{y}{y}|)}}$.
...
Of course, under this hypothesis $\NPEq=\coNPEq$, so the \coNPEq-complete problem exhibited in \autoref{thm:conpeqc} is also an \NPEq-complete problem.
\begin{openproblem}
