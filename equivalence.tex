...
\abstract{%
  ...
  We show that there is a complete problem under kernel reductions in \SPPEq.
...
% TODO add this in to show that there exists an infinite kernel reduction anti-chain of equivalence relations which are many-one equivalent
...
\begin{langdef}{EQUIV}
  \begin{instance}
    a polynomially clocked $\SigPi$ Turing machine and a positive integer $n$
  \end{instance}
  \begin{question}
...
\begin{lemma}
  If there exists a $\SigPi$ algorithm $A$ that decides whether $\pair{M}{n}$ is in \lang{EQUIV} within time $n^{f(|M|)}$ for some computable function $f$, then the language
  \begin{align*}
    R = {} & \{\pair{u}{v}|u=v\} \\
...
\begin{proof}
  ...
  Since the algorithm $A$ is a $\SigPi$ machine running in time $n^{f(|M|)}$, it runs in time polynomial in the length of input, so it is in $\SigPi$.
  Since $M$ is a $\SigPi$ machine, simulating $M$ one time is in $\SigPi$.
  Hence, $R\in\SigPi$.
  ...
  Finally, we must show that every equivalence relation $S$ in the class $\SPPEq$ reduces to $R$ under a polynomial time kernel reduction.
  ...
  We have shown that $R$ is both \SPPEq-hard and a member of \SPPEq, so $R$ is \SPPEq-complete.
\end{proof}
