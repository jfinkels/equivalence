...
\newcommand{\Pair}[2]{\big\langle#1,#2\big\rangle} % pairing function
...
\newcommand{\license}{\begin{tabular}[h]{r l}
    \bysa & \parbox{275pt}{Copyright 2011 Jeffrey Finkelstein. 
...
\begin{definition}
  Let $U$ be a set. Let $R\subseteq U\times U$. Then $R$ is an
    \defn{equivalence relation} on $U$ if and only if the following conditions
    hold: %\renewcommand{\labelenumi}{\roman{enumi}.}
  \begin{enumerate}
...
\begin{theorem}\label{thm:reduction_ce}
  Let $R$ and $S$ be equivalence relations, and let $R$ be decidable. If the
  sets of representatives of equivalence classes for both $R$ and $S$ are
  computably enumerable, then $R\krnt S$.
\end{theorem}
\begin{proof}
  Let $r_1, r_2, \ldots$ and $s_1, s_2, \ldots$ be enumerations of
  representatives of equivalence classes for $R$ and $S$ respectively. Let $M$
  be the Turing machine which decides $R$. Define the function $f$ as in
  \autoref{alg:reduction_ce}.
  \begin{algorithm}\label{alg:reduction_ce}
    \caption{Generic reduction for equivalence relations with computably
    enumerable representatives of equivalence classes}
    \KwIn{$w\in\Sigma^*$}
    \For{$i=1$ to $\infty$}{
      Simulate $M$ on input $\pair{w}{r_i}$\;
      If $M$ accepts, output $s_i$ and halt\;
    }
  \end{algorithm}

  Since the equivalence classes of $R$ partition $\Sigma^*$, each string in
  $\Sigma^*$ must relate to some representative in the enumeration, and since
  $M$ always halts, this procedure always halts.

  Suppose $\pair{x}{y}\in R$. Then $\pair{x}{r_i}\in R$ and $\pair{y}{r_i}\in
  R$, for some representative $r_i$. Then $f(x)=s_i$ and $f(y)=s_i$, so
  $\pair{f(x)}{f(y)}=\pair{s_i}{s_i}\in S$. For the converse, suppose
  $\pair{f(x)}{f(y)}\in S$. Therefore $\pair{f(x)}{s_i}\in S$ and
  $\pair{f(y)}{s_i}\in S$ for some representative $s_i$. By construction, $f$
  outputs $s_i$ if and only if its input relates to $r_i$ in $R$. Therefore
  $\pair{x}{r_i}\in R$ and $\pair{y}{r_i}\in R$, and hence $\pair{x}{y}\in
  R$. Therefore $\pair{x}{y}\in R$ if and only if $\pair{f(x)}{f(y)}\in S$.
\end{proof}
...
%% TODO I wish
%% We adapt this idea to create a set which is $\kr$-complete for \NPEq.

%% \begin{definition}
%%   Let the language $R_K$ be defined as follows:
%%   \begin{align*}
%%     R_K=\{\Pair{\triple{x}{M}{1^k}}{\triple{y}{M}{1^k}}|&M\plain{accepts}
%%     \pair{x}{y}\plain{in at most}k\plain{steps}\\
%%     &\plain{and}M\plain{accepts}\pair{y}{x}\plain{in at most}k\plain{steps}\}
%%   \end{align*}
...
\begin{theorem}\label{thm:diag}
  Let $R_1$ and $R_2$ be decidable equivalence relations, and let
  $\mathcal{C}_1$ and $\mathcal{C}_2$ be classes of decidable equivalence
  relations such that:
  \begin{enumerate}
  \item $R_1\notin\mathcal{C}_1$
  \item $R_2\notin\mathcal{C}_2$
