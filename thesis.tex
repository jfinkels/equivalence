...
\begin{proof}
  ...
  possibility that $i=j$). Now $M(x)$ and $M(y)$ will rearrange the bits of $x$
  and $y$, respectively, by stable sorting the bits of $a$. Call these new
  strings $x'$ and $y'$, and the corresponding permutation of indices $i$ and
  $j$, $i'$ and $j'$, respectively. If $a_{i'}$ is zero then $x_{i'}\neq
  y_{i'}$.
