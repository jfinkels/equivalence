...
\begin{definition}
  $\P$ is the class of languages for which membership can be decided in
  deterministic polynomial time.
  
  $\NP$ is the class of languages for which membership can be decided in
  nondeterministic polynomial time.

  $\NP$ can alternately be defined as the class of languages for which
  membership can be verified in deterministic polynomial time.
\end{definition}

\begin{definition}
  Let $A,B$ be languages. We say \emph{$A$ polynomial time, many-one reduces
    to $B$} if $\exists f\in\FP: \forall w\in\sigmastar, w\in A\iff f(w)\in
...
\begin{definition}
  ...
  If $(x,y)\in R$, we say \emph{$x$ relates to $y$} or \emph{$x$ is equivalent
    $y$ under $R$}. We denote this by $x\sim_R y$ or $x\sim y$.
\end{definition}
...
\begin{definition}
  ...
  \emph{equivalence class} of $x$ is $[x]_R=\{y|(x,y)\in R\}$.
\end{definition}
...
\begin{definition}\label{def:peq}
  ...
  be decided in nondeterministic polynomial time.

  \emph{\NPEq} can alternately be defined as the class of equivalence relations
  for which membership can be verified in deterministic polynomial time.
\end{definition}
...
\begin{definition}
  \emph{\FP} is the class of total computable functions which halt in
  deterministic polynomial time.
\end{definition}
...
\begin{proof}
  Since $R\kr S$, $\exists f\in\FP: (x,y)\in R\iff(f(x), f(y))\in S$. Define
  $g\in FP$ by $g((x,y))=(f(x), f(y))$. Therefore $R\mor S$ by $g$.
\end{proof}
...
reductions which follow the more general equivalence relation reduces to the
more restrictive equivalence relation.
...
elements, and it is its own equivalence class. The most restrictive is the
equality relation, $R_{eq}$, in which each binary string is the sole element in
its equivalence class. The other equivalence relations defined here also have a
(countably) infinite number of equivalence classes.
...
%% \begin{theorem}
%%   Let $R,S\in\PEq$. If $R$ has exactly two equivalence classes and $S$ has at
%%   least two equivalence classes, then $R\kr S$.
%% \end{theorem}
%% \begin{proof}
%%   Choose a representative of each equivalence class in $R$, call them $a$ and
%%   $b$. Choose a representative of two equivalence classes in $S$, call them $j$
%%   and $k$. Construct machine $M\in\FP$ on input $w\in\sigmastar$:
%%   \begin{algorithm}[H]
%%     \eIf{$w\in[a]$}{
%%       \Return{j}
%%     }{
...
%%   $M(x)=j$ and $M(y)=j$, so $(M(x),M(y))\in S$.
...
%%   classes. Then $R_{par}\kr S$.
%% \end{corollary}
%% \begin{corollary}
%%   $R_{par}\kr R_{eq}$
%% \end{corollary}

%% The kernel reductions we provide below from $R_{par}$ to $R_{bc}$ and then
%% to $R_{eq}$ somewhat obfuscate this ``decide then map'' process, but that is
...
\begin{proof}
  ...
  % In the case that $a'_{i'}=0$ then $x'_{i'}\neq y'_{i'}$, so $M(x)\neq M(y)$
  % and $(M(x),M(y))\notin R_{eq}$.
...
that the equality relation is not \PEq-complete. We restate this evidence after
...
problems decidable in polynomial time, whereas proofs for \P-completeness allow
only logarithmic space. This means we are allowed to examine the entire
computation history of the machine which decides any language in $\PEq$ (for
example, as the tableau of the deterministic polynomial time machine). And from
our examination of reductions among feasible equivalence problems, we can see
that some reductions seem to solve the problem to be reduced first. This
provides some intuition that polynomial time kernel reductions for problems in
$\PEq$ may be too powerful. Indeed, the real restriction here is on the number
...
\item combinatorial isomorphism of convex polytopes represented by
  vertex-facet incidences % wikipedia [26][27]
\item etc? % [17, 78, 94] 
