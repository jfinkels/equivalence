...
reduction maps $(x,y)\in R$ to $f(x, y)\in S$. Informally, a kernel reduction
gets access to both $x$ and $y$, but a kernel reduction gets access to only one
at a time. Because of the many-one reduction is more powerful, we can prove the
following lemma.
...
(the reductions were not explicitly presented as kernel reductions, but we can
retroactively identify some of the techniques used as such). If we can show a
polynomial time kernel reduction from graph isomorphism to $R_{KC}$, then we
have found many $\NPEq$ problems which reduce to $R_{KC}$. Fortunately,
$R_{KC}$ was constructed in such a way to allow such a kernel reduction to be
possible.
