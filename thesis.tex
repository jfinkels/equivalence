...
% package imports
\usepackage[noend,noline,linesnumbered,boxed,oldcommands]{algorithm2e} % for creating pseudocode
...
It appears that all of these ``bitwise'' equivalence problems reduce to the
...
unique strings, because each string is its own equivalence class, and so it
certainly has at least as many equivalence classes as any other equivalence
relation defined on binary strings. However, there is evidence\cite{fg09} that
the equality relation is not \PEq-complete. We restate this evidence after the
