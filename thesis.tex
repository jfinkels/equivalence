...
\begin{definition}
  ...
  Turing machine running in non-deterministic polynomial time.
...
\begin{definition}
  ...
  $w\in\sigmastar$.
\end{definition}
\begin{definition}\label{def:rbc}
...
\TSAT \cite{asu79}. Equivalence relations in $\NPEq\cap\NPC$ are easy to find;
we describe another in the proof of \autoref{thm:npeqc_neq_npc}.
...
We first provide this straightforward corollary of \autoref{thm:numbers}, which
gives some structure to \PEq- and \NPEq-complete problems, if they exist.

\begin{corollary}\label{cor:npeqc_infinite}
  \mbox{}
  \begin{enumerate}
    \renewcommand{\labelenumi}{\roman{enumi}.}
...
\begin{proof}
  ...
  producing a contradiction that $R$ is \PEq-complete. $R_{eq}$ has an infinite
  number of equivalence classes (specifically, one for each binary string), and
  $R$ has a finite number of equivalence classes, by assumption. By
...
%% TODO what is the significance of this theorem
