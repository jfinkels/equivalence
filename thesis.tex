...
\begin{abstract}
  ...
  examine some examples of kernel reductions among easy equivalence
  relations. We then examine the complexity of equivalence problems which are
  complete under kernel reductions in \NP. We also show that most known
  many-one reductions among isomorphism problems in $\NP$ are indeed kernel
  reductions.
\end{abstract}
...
relations in $\PEq$, tractible equivalence problems.
...
which should intuitively be easy because the graph isomorphism problem is
(seemingly) of greater complexity.
...
%% TODO evidence
There is evidence that shows that $GI$ is not \NP-complete. If this is true,
