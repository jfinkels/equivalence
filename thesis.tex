...
\end{definition}
\begin{definition}
  $R_{bc}=\{\pair{x}{y}|x$ and $y$ have the same number of ones$\}$. We use the
  subscript ``$bc$'' to denote that pairs of binary strings in this equivalence
  relation have the same bitcount (number of ones).
\end{definition}
\begin{definition}
...
\begin{proof}
  ...
  Suppose $\pair{x}{y}\in R_{eqi}$, so either $x=y$ or $x=\bar{y}$.
  ...
  Therefore $\pair{x}{y}\in R_{eqi} \iff \pair{M(x)}{M(y)}\in DirGI$, so
  $R_{eqi}\kr DirGI$.
\end{proof}
