...
\begin{proof}
  ...
  because the equivalence classes partition $\sigmastar$. Therefore this
  algorithm always outputs something on each input.

  Suppose $(x,y)\in R$, so $(x, r_i)\in R$ and $(y,r_i)\in R$ for some $r_i\in
  REP(R)$. Then $M(x)$ outputs $f(r_i)$ and $M(y)$ outputs $f(r_i)$, so $(M(x),
  M(y))=(f(r_i), f(r_i))\in S$.
