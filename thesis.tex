...
\section{Kernel reductions among feasible equivalence problems} \label{sec:p}
...
\begin{proof}
  ...
  Now suppose $A$ is also $\kr$-complete in $\NPEq$. Since
  $R_{bc}\in\PEq\subseteq\NPEq$, then $R_{bc}\kr A$.

  First, we will consider the equivalence classes in $R_{bc}$. There are a
  countably infinite number of equivalence classes in $R_{bc}$; they are $[0],
  [1], [11], [111], \ldots$. Each equivalence class is itself infinite as well;
  if $w\in\sigmastar$ then $[w]$ includes also $[0w], [00w], [000w],
  \ldots$.

  % TODO is the number of n vertex graphs correct

  There are an infinite number of equivalence classes in $A$, as well. However,
  each equivalence class contains only a finite number of elements. If
  $G=(V,E)$, then $[\pair{G}{k}]$ includes also all pairs $\pair{H}{k}$, where
  $G$ is isomorphic to $H$, and all pairs $\pair{J}{k}$, where $G$ and $J$ have
  the same number of vertices and both have a clique of size $k$. The set of
  all pairs $\pair{H}{k}$ is finite, because there are only a finite number of
  graphs on $|V|$ vertices (specifically, $2^\frac{|V|(|V|-1)}{2}$).
\end{proof}
